...
import { createClub, updateClub } from '../../services/privateApi';
...
function UpdateClubForm({ id, setUpdateForm }) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const response = await updateClub(file, formData, id);
            if (response) {
                setNotify({
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                setFileList([{
                    uid: '-1',
                    name: result?.result?.thumbnail.alt,
                    status: 'done',
                    url: `http://localhost:8080/api/v1/images/${result?.result?.thumbnail.id}`,
                }])
            } catch (err) {
                console.log(err);
            }
    ...
    return (
        <>
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", marginTop: "16px", gap: "5px" }}>
                <Button onClick={() => setUpdateForm(false)}>Huỷ</Button>
